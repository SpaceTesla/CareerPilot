...
- **Backend (local):** Python 3.13; [`uv`](https://docs.astral.sh/uv/) recommended
- **Frontend (local):** Node.js **20+** (matches the production Docker image)
- **Keys:** `GOOGLE_API_KEY` (Gemini) and `TAVILY_API_KEY` — see `backend/.env.example`
...
Typical long-running services after `up`:
...
For routes that use browser automation locally, install Chromium once (from `backend/`):
